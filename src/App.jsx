...
function App() {
  ...
  const fetchNews = async (query = '', isSearch = false) => {
    // 2. Prevent crash if key is missing
    if (!API_KEY) {
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // 3. Handle API limit or empty results
      if (data.results) {
  ...
  if (!API_KEY && !loading) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-[#dce9ff] p-6 text-center">
        <div className="text-6xl mb-4">🔑</div>
        <h1 className="text-2xl font-black uppercase tracking-tighter">Configuration Required</h1>
        <p className="text-gray-600 mt-2 max-w-md">
          Please add <b>REACT_APP_NEWS_API_KEY</b> to your Environment Variables in Vercel to see the news.
        </p>
      </div>
    );
  }
      ...
      <header className={`${darkMode ? 'bg-[#0c0c0c] border-white/10' : 'bg-white border-gray-100'} border-b px-4 md:px-6 py-4 sticky top-0 z-50 transition-colors`}>
        <div className="max-w-7xl mx-auto flex justify-between items-center relative">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            <div className="hidden md:flex w-8 h-8 bg-black rounded-full items-center justify-center text-white text-[10px] font-bold border border-white/20">GO</div>
          </div>
        </div>
          ...
          <>
            {articles.length > 0 ? (
              ...
              <div className="h-[40vh] flex flex-col items-center justify-center text-center">
                <p className="text-xl font-bold opacity-50 uppercase tracking-widest">No articles found in this category.</p>
              </div>
            )}
            ...
            <footer className={`${darkMode ? 'bg-white text-black' : 'bg-[#0c0c0c] text-white'} -mx-4 md:-mx-6 px-6 md:px-10 py-16 md:py-24 mt-20 transition-colors`}>
              ...
              <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 md:gap-20">
                <div className="space-y-6">
                  ...
                  <p className={`text-xs md:text-sm font-medium leading-loose ${darkMode ? 'text-gray-600' : 'text-gray-500'}`}>Ghana’s digital pulse. High-performance news built in Accra.</p>
                </div>
                <div>
                  ...
                  <ul className="text-[10px] md:text-xs space-y-4 md:space-y-5 font-black uppercase tracking-widest">
                    {sections.map(s => <li key={s} onClick={() => setCategory(s)} className="hover:text-[#aa3bff] cursor-pointer">{s}</li>)}
                  </ul>
                </div>
                <div>
